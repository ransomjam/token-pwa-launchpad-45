...
import { VendorDistributionSheet } from './VendorDistributionSheet';
...
export const CreateActionSheet = ({ open, onOpenChange }: CreateActionSheetProps) => {
  const { t } = useI18n();
  const [distributionType, setDistributionType] = useState<'auction' | 'listing' | null>(null);

  const handleCreateAuction = () => {
    trackEvent('vendor_auction_create');
    onOpenChange(false);
    setDistributionType('auction');
  };

  const handleCreateListing = () => {
    trackEvent('vendor_listing_create');
    onOpenChange(false);
    setDistributionType('listing');
  };
  ...
  return (
    <>
      ...
      <VendorDistributionSheet
        open={distributionType !== null}
